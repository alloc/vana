...
import { isDraft as isImmerDraft } from '../immer'
import {
  __$observable,
  getObservable,
  isObservable,
  ...
  const observeTree = (parent: object, observer: OProps) => {
    ...
    Object.keys(parent).forEach(prop => {
      ...
      if (isObject(value)) {
        ...
        if (!observable) {
          if (isFrozen(value) || !isCopyable(value)) return
